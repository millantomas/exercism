...
export function isValidCommand(command) {
  ...
  const test = command.match(regex);
...
export function getURL(userInput) {
  const regex = /\b(?:https?:\/\/)?([a-zA-Z0-9-]+\.[a-zA-Z0-9-]+(?:\.[a-zA-Z]{2,})?)\b/g;
...
export function niceToMeetYou(fullName) {
  ...
  const result = fullName.replace(fullName, `Nice to meet you, ${name[1]} ${name[0]}`)
